...
void MainWindow::positionChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Position : GraphicScene::A_Nothing);
    }
}

void MainWindow::transitionChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Transition : GraphicScene::A_Nothing);
    }
}

void MainWindow::moveChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Move : GraphicScene::A_Nothing);
    }
}

void MainWindow::connectChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Connection : GraphicScene::A_Nothing);
    }
}

void MainWindow::rotateChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Rotation : GraphicScene::A_Nothing);
    }
}

void MainWindow::removeChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Remove : GraphicScene::A_Nothing);
    }
}

void MainWindow::markerChecked(bool checked) {
    if (auto scene = currentScene(); scene) {
        scene->setMode(checked ? GraphicScene::A_Nothing : GraphicScene::A_Nothing);
    }
}
